...
\begin{align}
\Pr\brak{M=k} = {{n}\choose{k}} * (l)^{n-k} * (r)^{k}\label{0.0.1}
\end{align}
Where
\begin{itemize}
...
\begin{align}
\Pr\brak{M=k} = {{n}\choose{k}} * \left(\frac{1}{10}\right)^{n-k} * \left(\frac{1}{10}\right)^{k}
\end{align}
...
\bigskip
\begin{align}
\sum_{M=1}^{10} Pr(M) = 1\label{0.0.3}
\end{align}
\bigskip
Thus, we can write:
\begin{align}
1 - \Pr\brak{M \geq 7} = 1 - \sum_{M=7}^{10} \Pr\brak{M}\label{0.0.4}
\end{align}

\begin{align}
\implies 1 - \sum_{k=7}^{10} {{10}\choose{k}} * {0.1}^{10-k} * {0.9}^{k}\label{0.0.5}
\end{align}
...
\begin{align}
\implies \Pr\brak{M \leq 6} = 0.012795198\notag
\end{align}
\bigskip\\
